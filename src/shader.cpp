...
shader::shader(
    const std::string& sVertexFPath,
    const std::string& sFragmentFPath) {
...
std::optional<GLuint>
shader::s_compileShader(const GLenum type, const std::string& sShader) {
    ...
    GLCALL(
        glGetShaderInfoLog(shaderId, error_length, &error_length, &sError[0]));
    PrintError(
        "(CompileShader) : Cannot Create Shader.\n ErrorLog = " + sError);

    return {};
}

bool shader::m_createShader(
...
bool shader::m_createShaders(
    const std::string_view& sVertexFPath,
    const std::string_view& sFragmentFPath) {
    GLCALL(m_id = glCreateProgram());

    const bool result = m_createShader(sVertexFPath, GL_VERTEX_SHADER)
