...
{   
    ...
    public:
        ...
        constexpr vec4<T> operator+ (const vec4<T>& v) const noexcept { return { x + v.x, y + v.y, z + v.z, w + v.w }; }    
        constexpr vec4<T> operator- (const vec4<T>& v) const noexcept { return { x - v.x, y - v.y, z - v.z, w - v.w }; }
        constexpr vec4<T> operator* (const vec4<T>& v) const noexcept { return { x * v.x, y * v.y, z * v.z, w * v.w }; }    
        constexpr vec4<T> operator/ (const vec4<T>& v) const noexcept { return { x / v.x, y / v.y, z / v.z, w / v.w }; }
        
        constexpr vec4<T>& operator+= (const vec4<T>& v) noexcept { return *this = *this + v; }
        constexpr vec4<T>& operator-= (const vec4<T>& v) noexcept { return *this = *this - v; }
        constexpr vec4<T>& operator*= (const vec4<T>& v) noexcept { return *this = *this * v; }
        constexpr vec4<T>& operator/= (const vec4<T>& v) noexcept { return *this = *this / v; }
        ...
        { 
            if (i >= 4) throw "vec4<T>::operator[] out of range index";
